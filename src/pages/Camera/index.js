...
const Camera = () => {
    ...
    useEffect(() => {
        ...
        const getVideoInput = async () => {
            ...
            select.forEach((element, index) => {
                array.push({ value: `Camera ${index + 1}`, key: element.deviceId });
            });
    ...
    useEffect(() => {
        if (mediaStream && videoRecordRef.current && !videoRecordRef.current.srcObject) {
            ...
            <div className='grid-primary'>
                ...
                <div className='start-1 size-6 padding-v-20'>
                    <UI.Selector
                        ...
                        options={videoSource}
                        titleTop='Cameras'
                        eventChange={seleccion => onchange('videoinput', seleccion.target.value)}
                        value={videoinput}
                        disabled={disable}
                    />
                </div>
                <div className='size-3 padding-v-20'>
                    <UI.Selector
                        ...
                        eventChange={seleccion => onchange('outputFormat', seleccion.target.value)}
                ...
                <div className='start-1 size-6 padding-v-20'>
                    ...
                    <div className='camera-container'>
                        <video ref={videoRecordRef} autoPlay playsInline muted />
                        <div className='container-controls-rec'>
                            <div className="controls-rec center">
                                {recording ?
                                    <>
                                        <div className='rec on' />
                                        <p className='font-white strong'> Reconding... </p>
                                    </>
                                    : <></>
                ...
                <div className='start-1 size-6 padding-v-20'>
                    ...
                    <div className='photo center' >
                        ...
                        {state.imageURL ? <img src={state.imageURL} alt='imageURL' /> : <></>}
                    </div>
                </div>
                <div className='size-6 padding-v-20'>
                    <h4>Logs</h4>
                    <textarea className='log' readOnly={true} value={log.join('')} />
                </div>

                <div className=' start-1 size-2 padding-v-20'>
                    <UI.Button title='Start Recording' onClick={() => {startRecording(); setRecording(true);}} disabled={disableButtons} />
                </div>
                <div className=' size-2 padding-v-20'>
                    <UI.Button title='Stop Recording' onClick={() => {stopRecording(); setRecording(false);}} disabled={disableButtons} />
                </div>
                <div className=' size-2 padding-v-20'>
                    <UI.Button title='On Camera' onClick={() => { OnCamera(); setRecording(false); }} disabled={disableButtons} />
                </div>
                <div className=' size-2 padding-v-20'>
                    <UI.Button title='Off Camera' onClick={() => { offCamera(); setRecording(false); }} disabled={disableButtons} />
                </div>
                <div className=' size-2 padding-v-20'>
                    <UI.Button title='Take a photo' onClick={() => takePhoto()} disabled={disableButtons} />
                </div>
                <div className=' size-1 padding-v-20'>
                    <UI.Button title='Play' onClick={() => playVideo()} disabled={!mediaBlob} />
                </div>
                <div className=' size-1 padding-v-20'>
                    <UI.Button title='Stop' onClick={() => stopVideo()} disabled={!mediaBlob} />
                </div>
